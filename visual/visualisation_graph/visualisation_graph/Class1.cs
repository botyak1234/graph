...
namespace visualisation_graph
{
    internal class Graph
    {
        ...
        public void GenerateRandomGraph(int vertexCount, int edgeCount)
        {
            ...
            var vertices = Enumerable.Range(0, vertexCount).Select(i => $"V{i}").ToList();
        ...
        public List<string> FindShortestPath(string start, string end, string algorithm)
        {
            switch (algorithm)
            {
                case "Dijkstra":
                    var dijkstraResult = Dijkstra(start);
                    return ReconstructPath(start, end, dijkstraResult);

                case "BellmanFord":
                    var bellmanFordResult = BellmanFord(start, out var predecessors);
                    return ReconstructPath(start, end, bellmanFordResult, predecessors);

                case "FloydWarshall":
                    bool hasNegativeCycle;
                    var floydResult = FloydWarshallWithCycleCheck(out hasNegativeCycle);

                    if (hasNegativeCycle)
                        throw new InvalidOperationException("Граф содержит отрицательный цикл.");

                    return ReconstructPath(start, end, floydResult);
            }
        ...
        private List<string> ReconstructPath(string start, string end, Dictionary<string, int> distances)
        {
            if (!distances.ContainsKey(end) || distances[end] == int.MaxValue)
                return new List<string>(); // Путь не существует.
            ...
            {
                path.Add(current);
                current = distances[current] == 0 ? null : adjacencyList[current].FirstOrDefault(x => distances[x.Item1] + x.Item2 == distances[current])?.Item1;
            }
        ...
        private List<string> ReconstructPath(string start, string end, Dictionary<string, int> distances, Dictionary<string, string> predecessors)
        {
            if (!distances.ContainsKey(end) || distances[end] == int.MaxValue)
                return new List<string>(); // Путь не существует.
        ...
        private List<string> ReconstructPath(string start, string end, Dictionary<string, Dictionary<string, int>> distances)
        {
            if (!distances.ContainsKey(start) || !distances[start].ContainsKey(end) || distances[start][end] == int.MaxValue)
                return new List<string>(); // Путь не существует.
            ...
            while (current != end)
            {
                ...
                {
                    if (distances[start][neighbor.Item1] + neighbor.Item2 == distances[start][end])
                    {
                        path.Add(neighbor.Item1);
        ...
        public bool IsNeighbor(string vertex1, string vertex2)
        {

            if (adjacencyList.ContainsKey(vertex1))
            {
                foreach (var edge in adjacencyList[vertex1])
                {
                    if (edge.Item1 == vertex2)
                        return true;
                }
            }
            return false;
